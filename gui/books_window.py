...
from Roots.models import Book, Member
from Roots.daos import list_books, add_book, update_book, delete_book, borrow_book
...
class BooksWindow(QDialog):
    ...
    def __init__(self, parent=None, librarian_name: str = "", member: Member = None):
        ...
        self.table.setHorizontalHeaderLabels(
            ["ID", "Title", "Author", "Genre", "Total copies", "Available"]
        )
    ...
    def _load_books(self):
        ...
        for row, book in enumerate(books):
            ...
            self.table.setItem(row, 2, QTableWidgetItem(book.author_name))
            self.table.setItem(row, 3, QTableWidgetItem(book.genre))
            self.table.setItem(row, 4, QTableWidgetItem(str(book.total_copies)))
            self.table.setItem(row, 5, QTableWidgetItem(str(book.available_copies)))
    ...
    def _handle_add(self):
        ...
        add_book(title.strip(), author.strip(), genre.strip(), copies)
        self._load_books()
